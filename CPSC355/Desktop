...
m_done:	adrp	x0, line9
	add	x0, x0, :lo12:line9
	bl	printf
...
equal:	stp	fp, lr, [sp, equal_alloc]!		// Allocate space for the int result
	...
	mov	w13, FALSE				// Loading FALSE
	str	w13, [fp, result_s]			// @@@ int result = FALSE;
...
letsgo:	mov	w13, TRUE				// Loading True
	str	w13, [fp, result_s]			// @@@ int result = TRUE;

eq_dne:	ldr	w0, [fp, result_s]			// return result;
	ldp	fp, lr, [sp], equal_dealloc		// Deallocating memory for equalSize()
	ret						// Return to call

expand:	stp	fp, lr, [sp, -16]!
	mov	fp, sp

	ldr 	w9, [x0, pyramid_size + size_width]
	mul	w9, w1, w9
	str	w9, [x0, pyramid_size + size_width]
	
	ldr	w10, [x0, pyramid_size + size_length]
	mul	w10, w1, w10
	str 	w10, [x0, pyramid_size + size_length]
	
	ldr	w11, [x0, pyramid_height]
	mul	w11, w1, w11
	str	w11, [x0, pyramid_height]
	...
	str	w12, [x0, pyramid_volume]

	ldp	fp, lr, [sp], 16
	ret

relo:	stp	fp, lr, [sp, -16]!
	mov 	fp, sp
	
	ldr	w9, [x0, pyramid_coord + coord_x]
	add	w9, w1, w9
	str	w9, [x0, pyramid_coord + coord_x]

	ldr	w10, [x0, pyramid_coord + coord_y]
	add	w10, w2, w10
	str	w10, [x0, pyramid_coord + coord_y]
